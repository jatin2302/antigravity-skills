...
2. Read project `gemini.md` → load project identity, rules, success metrics
3. Read `progress.md` → identify next action and blockers
4. Read `task_plan.md` → identify current sprint and task statuses
...
When ending a session (completing an Epic, hitting a blocker, or pausing), you MUST leave a clean handoff for the next agent instance by appending this block to the bottom of the `progress.md` file:
...
## Anti-Patterns
- **Don't** assume what the project is doing; always read `gemini.md` & `task_plan.md`.
- **Don't** write 300 lines of setup code without the Pre-Flight Gate.
- **Don't** yield with "I'm done" without logging the Context Handoff to `progress.md`.
- **Don't** create meta-files (`gemini.md`, `progress.md`, `findings.md`, `task_plan.md`) in the root directory. They MUST go in ``.
