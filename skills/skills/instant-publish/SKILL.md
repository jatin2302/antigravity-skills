...
description: End-to-end workflow for pushing a web project to GitHub and deploying it live on Vercel with zero hiccups. Incorporates all known gotchas and best practices.
---
...
**Vite**
- Static assets must be in `public/` — the #1 deployment issue
- `base` in `vite.config.ts` — should be `'/'` for Vercel (default is fine)
- SPA routing — add `vercel.json` with rewrites if using client-side routing:
```json
{
  "rewrites": [
...
**Next.js**
- Static exports — if using `output: 'export'`, set output directory in Vercel
- API routes — work automatically on Vercel as serverless functions
- Image optimization — works out of the box on Vercel
