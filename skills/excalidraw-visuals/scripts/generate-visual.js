 ...
 * Requires KIE_AI_API_KEY in .env file at project root (relative to this script)
 */
...
// ADJUSTED: Reaching project root from skills/excalidraw-visuals/scripts
    ...
    for (const line of envContent.split('\n')) {
        ...
        if (trimmed && !trimmed.startsWith('#')) {
            ...
            if (eqIndex > 0) {
                ...
                process.env[key] = value.trim().replace(/^['"](.*)['"]$/, '$1'); // trim and strip quotes
            }
        }
    }
}
...
if (!API_KEY) {
    console.error('Error: KIE_AI_API_KEY not set. Add it to .env file in project root.');
    process.exit(1);
}
